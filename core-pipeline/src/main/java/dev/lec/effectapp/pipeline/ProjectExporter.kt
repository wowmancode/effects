...
class ProjectExporter(private val context: Context) {
 ...
 fun startIhtx(project: EditProject,exports:Int,lengthMs:Long,outputPath:String,callback:Callback,overlays:List<Overlay> = emptyList(),overlayGridSize:Int = 2) {
  ...
  val plan=IhtxPlan(base,overlays,passes,overlayGridSize.coerceAtLeast(1),videoSize(base.clips.first().sourceUri))
 ...
 private fun startStage(b:Batch,index:Int){ transformer.start(ProjectCompositionFactory.create(b.projects[index],resolveBitmap,if (b.ihtx != null) 30 else null,b.ihtx?.overlayFrameSize),b.files[index].absolutePath) }
 ...
 private data class IhtxPlan(val base:EditProject,val overlays:List<Overlay>,val passes:Int,val gridSize:Int,val overlayFrameSize:Size?) {
